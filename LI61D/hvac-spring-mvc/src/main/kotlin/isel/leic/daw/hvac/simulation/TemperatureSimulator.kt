...
private class TemperatureActuator(val action: TimerTask.() -> Unit) {
    ...
    fun turnOn() {
        if (isOn) return
        timer = fixedRateTimer(null, false, FIVE_SECS, FIVE_SECS, action)
    }
...
class HeaterSimulator(sensor: SensorSimulator) : Heater {
    ...
    private val actuator = TemperatureActuator {
...
class CoolerSimulator(sensor: SensorSimulator) : Cooler {
    ...
    private val actuator = TemperatureActuator {
        logger.info("Cooler is running and current temperature is ${sensor.temperature}")
        sensor.temperature -= 1
    }
